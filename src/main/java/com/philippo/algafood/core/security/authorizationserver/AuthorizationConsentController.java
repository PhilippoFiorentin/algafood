...
public class AuthorizationConsentController {
    ...
    public String consent(Principal principal,
        ...
        if (client == null) {
            throw new AccessDeniedException(String.format("Client with the %s was not found", clientId));
        }
        ...
        Set<String> scopesToAprove = new HashSet<>(Set.of(scopeArray));

        Set<String> previouslyApprovedScopes;

        if (consent != null) {
            previouslyApprovedScopes =  consent.getScopes();
            scopesToAprove.removeAll(previouslyApprovedScopes);
        } else{
        ...
        model.addAttribute("scopesToApprove", scopesToAprove);
